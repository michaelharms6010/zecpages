        ...
        <div style={{position: "relative"}} className="publish-page">
            ...
            <form className="publish-form" onSubmit={handleSubmit}>
                ...
                <textarea maxLength="500"
                    value={memo}
                    name="memo"
                    disabled={(user_id && lastArticle && lastArticle.date_created && new Date(lastArticle.date_created).getTime() > Date.now() - (1000 * 60 * 60 * 2)) || !subscribers.length || done}
                    onChange={e => setMemo(e.target.value)} />}
                <button disabled={(user_id && lastArticle && lastArticle.date_created && new Date(lastArticle.date_created).getTime() > Date.now() - (1000 * 60 * 60 * 2)) || !subscribers.length || done} type="submit">{subscribers.length ? "Publish To Subscribers" : "You need subscribers to publish"}</button>
            </form>
